...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if(src1 >= src2){s->dnpc = s->pc; s->dnpc += imm;});
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if(((uint64_t)src1) < ((uint64_t)src2)){s->dnpc = s->pc; s->dnpc += imm;});
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, R(rd) = SEXT(((uint32_t)src1) % ((uint32_t)src2), 32));//？？？待验证，含义不明确
