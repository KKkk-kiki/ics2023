...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw   , R, R(rd) = (uint32_t)((uint32_t)src1 >> BITS(src2, 4, 0)));//？
  INSTPAT("000000 ?????? ????? 101 ????? 00110 11", srliw  ,SI, R(rd) = (uint32_t)((uint32_t)src1 >> BITS(imm, 4, 0)));
  INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai   ,SI, R(rd) = (int32_t)src1 >> imm);
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(rd) = (uint32_t)((int32_t)src1 >> BITS(src2, 4, 0)));
  INSTPAT("010000 ?????? ????? 101 ????? 00110 11", sraiw  ,SI, R(rd) = SEXT((int32_t)src1 >> imm, 64));
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, R(rd) = SEXT(((uint32_t)src1 ) << (src2&0x1F), 64));//???待测试
  ...
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = imm );//待验证
