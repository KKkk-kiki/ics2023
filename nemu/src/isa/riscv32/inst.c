...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if(((int64_t)src1) >= ((int64_t)src2)){s->dnpc = s->pc; s->dnpc += imm;});
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if(((int64_t)src1) < ((int64_t)src2)){s->dnpc = s->pc; s->dnpc += imm;});//???
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1 != src2){s->dnpc = s->pc; s->dnpc += imm;});
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw   , R, R(rd) = SEXT(src1 * src2, 32));
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, R(rd) = SEXT(((int32_t)src1) % ((int32_t)src2), 32));//？？？待验证，含义不明确
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(rd) = SEXT(src1 + imm, 32));//待验证
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = SEXT(Mr(src1 + imm, 4), 32));   //待验证,已测试
